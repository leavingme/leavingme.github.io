...
Form.Element.Serializers = {
    ...
    select: function (element, index) {
        if (index === undefined) return this[element.type == 'select-one' ? 'selectOne' : 'selectMany'](element);
        else {
...
function utf8encode(s) {
    ...
    for (var i = 0; i < s.length; ++i) {
        ...
        if (firstSurrogate != 0) {
            ...
            if ((code < 0xD800) || (code > 0xDFFF)) {} else if ((code >= 0xD800) && (code < 0xDC00)) {
                firstSurrogate = code;
                continue;
            } else {
                continue;
...
function fixupIEPNGBG(oBlock) {
    if (oBlock) {
        if (elementHasCSSBGPNG(oBlock)) {
            ...
            var filterStyle = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + imageURL + "', sizingMethod='crop');";
            var fixupIEPNGBG_helper = function (img) {
                ...
                var extraHeight = 0;
                var extraWidth = 0;
                if (((currentBGRepeat.indexOf("repeat-x") != -1) && (tileWidth == 1) && (tileHeight == blockHeight)) || ((currentBGRepeat.indexOf("repeat-y") != -1) && (tileWidth == blockWidth) && (tileHeight == 1)) || ((currentBGRepeat == "repeat") && (tileWidth == 1) && (tileHeight == 1))) {
                    tileWidth = blockWidth;
                    tileHeight = blockHeight;
                    filterStyle = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + imageURL + "', sizingMethod='scale');";
        ...
        if (oNode && (parseFloat(oNode.currentStyle.getAttribute('opacity')) < 1)) {
            ...
            if (oNode.tagName.toLowerCase() == 'img') {
                ...
                if (absolutelyPositioned) {
                    ...
                    targetNode.setStyle({
                        marginTop: px((parseFloat(oNode.getStyle("marginTop")) || 0) + extents.top),
                        marginLeft: px((parseFloat(oNode.getStyle("marginLeft")) || 0) + extents.left),
                        marginBottom: px((parseFloat(oNode.getStyle("marginBottom")) || 0) - (extents.bottom - oNodeHeight)),
                        marginRight: px((parseFloat(oNode.getStyle("marginRight")) || 0) - (extents.right - oNodeWidth))
                    });
                }
                oNode.setStyle({
                    position: "absolute",
...
function getShrinkableParaDescendants(oAncestor) {
    return $(oAncestor).select('div.paragraph, p').findAll(function (paragraph) {
...
function joltLater(element) {
    setTimeout(function (element) {
        $(element).hide();
    }.bind(null, element), 100);
    setTimeout(function (element) {
        $(element).show();
    }.bind(null, element), 200);
}
...
function iWPosition(abs, left, top, width, height) {
    ...
    var size = "";
    if (width && height) size = ' width: ' + width + 'px; height: ' + height + 'px;';
...
function imgMarkup(src, style, attributes, alt, forceFixupIE7) {
    ...
    if (src) {
        ...
        attributes = attributes || "";
        alt = alt || "";
        if (windowsInternetExplorer && ((effectiveBrowserVersion < 7) || (effectiveBrowserVersion < 8 && forceFixupIE7 !== false))) {
...
var IWSize = Class.create({
    ...
    scaleToTargetSize: function (targetSize, fitTarget) {
        ...
        if (scaledSize.width > 0 && scaledSize.height > 0) {
            ...
            var scale = fitTarget ? Math.min(wScale, hScale) : Math.max(wScale, hScale);
...
var IWRect = Class.create({
    ...
    position: function () {
        return iWPosition(true, this.origin.x, this.origin.y, this.size.width, this.size.height);
    },
    clip: function () {
...
function adjustNodeIds(node, suffix) {
    ...
    $(node).childElements().each(function (e) {
...
function substituteSpans(parentNode, replacements) {
    $H(replacements).each(function (pair) {
        ...
        $(parentNode).select(selector).each(function (node) {
...
Element.addMethods({
    ...
    setFilter: function (element, filterName, filterParams) {
        ...
        element.style.filter = element.style.filter.replace(regex, '') + filterName + '(' + filterParams + '); ';
        return element;
    },
...
Object.extend(String.prototype, {
    ...
    pathExtension: function () {
        ...
        return ((lastDotIndex > lastSeparatorIndex + 1) && lastDotIndex > 0) ? this.slice(lastDotIndex + 1) : this;
    },
    ...
    stringByDeletingPathExtension: function () {
        var lastSeparatorIndex = this.lastIndexOf("/");
        var lastDotIndex = this.lastIndexOf(".");
        if ((lastDotIndex > lastSeparatorIndex + 1) && lastDotIndex > 0) return this.slice(0, lastDotIndex);
        return this;
    },
    ...
    httpURLRegExp: function () {
        if (String.m_httpurlRegExp == undefined) {
            ...
            var digits = digit + "+";
            var hostnumber = "(" + digits + "\\." + digits + "\\." + digits + "\\." + digits + ")";
            var toplabel = "((" + alpha + "(" + alpha + "|" + "-)*" + alphadigit + ")|" + alpha + ")";
            ...
            var httpurl = "((http)|(feed)|(https))://" + hostport + "(/" + hpath + "(\\?" + search + ")?)?"
    ...
    plaintextgsub: function (pattern, replacement) {
        ...
        while (true) {
            ...
            if (index == -1) break;
            value = value.substr(0, index) + replacement + value.substr(index + pattern.length);
        }
        return value;
    }
});
...
Object.extend(IWURL.prototype, {
    ...
    toURLString: function () {
        if (this.isRelative()) {
            return this.encodedPath() + (this.mQuery ? "?" + this.mQuery : "") + (this.mFragment ? "#" + this.mFragment : "");
        } else {
            return this.mProtocol + ":" + "//" + this.mAuthority + this.encodedPath() + (this.mQuery ? "?" + this.mQuery : "") + (this.mFragment ? "#" + this.mFragment : "");
        }
    },
    isEqual: function (that) {
    ...
    relativize: function (base) {
        if (base && (base.mPathComponents && base.mPathComponents.length > 0) && (this.mProtocol == base.mProtocol) && (this.mAuthority == base.mAuthority)) {
            ...
            for (var index = 0; index < Math.min(this.mPathComponents.length, base.mPathComponents.length); ++index) {
                if (this.mPathComponents[index] == base.mPathComponents[index]) commonAncestorIndex = index;
                else break;
            }
            var relativePath = [];
