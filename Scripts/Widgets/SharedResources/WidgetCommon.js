...
var Widget = Class.create({
    ...
    showView: function(viewName){
        ...
        if ((futureView != this.m_currentView) && (futureView != this.m_futureView)) {
            ...
            this.m_fadeAnimation = new SimpleAnimation(function(){
                delete this.m_fadeAnimation;
            }
.bind(this));
            this.m_fadeAnimation.pre = function(){
                if (previousView) {
                    previousView.ensureDiv().setStyle({
                        zIndex: 0,
                        opacity: 1
                    });
                }
                if (currentView) {
                    currentView.ensureDiv().setStyle({
                        zIndex: 1,
                        opacity: 0
                    });
                    currentView.show();
            ...
            this.m_fadeAnimation.post = function(){
                ...
                !currentView || currentView.ensureDiv().setStyle({
                    zIndex: '',
                    opacity: 1
                });
    ...
    croppingDivForImage: function(image, kind, width){
        ...
        if (image.loaded()) {
            ...
            var croppingDivForImage_helper = function(loadedImage){
                ...
                var offset = new IWPoint(Math.abs(scaled.width - cropped.width) / 2, Math.abs(scaled.height - cropped.height) / 2);
                img.setStyle({
...
IWScrollbar.prototype.setScrollArea = function(scrollarea){
    if (this.scrollarea) {
        Event.stopObserving(this.scrollbar, "mousewheel", this.scrollarea._mousewheelScrollHandler, true);
        Event.stopObserving(this.scrollbar, "DOMMouseScroll", this.scrollarea._mousewheelScrollHandler, true);
    }
    this.scrollarea = scrollarea;
    Event.observe(this.scrollbar, "mousewheel", this.scrollarea._mousewheelScrollHandler, true);
    Event.observe(this.scrollbar, "DOMMouseScroll", this.scrollarea._mousewheelScrollHandler, true);
}
...
function IWScrollArea(content){
    ...
    Event.observe(this.content, "mousewheel", this._mousewheelScrollHandler, true);
    Event.observe(this.content, "DOMMouseScroll", this._mousewheelScrollHandler, true);
    this.refresh();
...
IWScrollArea.prototype.remove = function(){
    Event.stopObserving(this.content, "mousewheel", this._mousewheelScrollHandler, true);
    Event.stopObserving(this.content, "DOMMouseScroll", this._mousewheelScrollHandler, true);
    var scrollbars = this._scrollbars;
    var c = scrollbars.length;
    for (var i = 0; i < c; ++i) {
...
IWScrollArea.prototype.mousewheelScroll = function(event){
    var deltaScroll = event.wheelDelta ? (event.wheelDelta / 120 * this.singlepressScrollPixels) : (event.detail / -2 * this.singlepressScrollPixels);
...
var StatusView = Class.create(View, {
    ...
    render: function(){
        ...
        if (this.upperRightBadgeWidth && this.upperRightBadgeHeight) {
            var badgeURL = (this.upperRightBadge) ? (this.m_widget.widgetPath + "/" + this.upperRightBadge) : transparentGifURL();
            markup += imgMarkup(badgeURL, "", "class='StatusUpperRightBadge' width='" + this.upperRightBadgeWidth + "' height='" + this.upperRightBadgeHeight + "' ", "");
        }
        var overlayPath = this.m_widget.sharedPath.stringByAppendingPathComponent("Translucent-Overlay.png");
        markup += imgMarkup(overlayPath, "position: absolute; top: 0; left: 0;", "id='" + this.p_overlayImgId() + "' width='700' height='286' ", "");
